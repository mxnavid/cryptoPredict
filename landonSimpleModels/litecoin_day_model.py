...
def litecoin_dayModel(outputFileName):
    ...
    df = web.get_data_yahoo('LTC-USD', start="2015-09-09", end=today)
    ...
    fig.map(plt.scatter, 'Date', 'market_cu_return', s=3).add_legend()
    plt.show()
    ...
    df.to_csv(os.path.join(path, 'DAILYLitecoin_model_output.csv'),encoding = 'utf-8', index = False)
...
litecoin_dayModel('../ui/src/scraped/litecoin/Litecoin_day_model_output.js')
